...
class OverviewScreenB1Test {
  ...
  fun todoListDisplaysAssigneeName() {
    ...
    composeTestRule
        .onNodeWithTag(OverviewScreenTestTags.getTestTagForTodoItem(todo1))
        .assertTextContains(todo1.assigneeName)
  }
  ...
  fun todoListDisplaysDueDate() {
    ...
    composeTestRule
        .onNodeWithTag(OverviewScreenTestTags.getTestTagForTodoItem(todo1))
        .assertTextContains(dueDate, substring = false, ignoreCase = false)
  }

  @Test
  fun todoListDisplaysStatus() {
    val todoList = listOf(todo1.copy(status = ToDoStatus.CREATED))
    setContent(withRepository = ToDosRepositoryImpl(todoList = todoList))

    composeTestRule
        .onNodeWithTag(OverviewScreenTestTags.getTestTagForTodoItem(todo1))
        .assertTextContains(ToDoStatus.CREATED.toString(), substring = false, ignoreCase = true)
  }
  ...
  fun todoListDisplaysExistingTodos() {
    ...
    todoList.forEach {
      val node = composeTestRule.onNodeWithTag(OverviewScreenTestTags.getTestTagForTodoItem(it))
      node.assertIsDisplayed()
      node.assertTextContains(it.name)
      node.assertTextContains(it.assigneeName)
      node.assertTextContains(it.status.displayString())
    }
  }
  ...
  fun dueDateIsCorrectlyFormatted() {
    ...
    composeTestRule
        .onNodeWithTag(OverviewScreenTestTags.getTestTagForTodoItem(todo1))
        .assertTextContains(dueDate1, substring = false, ignoreCase = false)
  }

  @Test
  fun canScrollOnTheTodoList() {
    val todos =
        (1..20).toList<Int>().map { todo1.copy(uid = it.toString(), name = "${todo1.name} #$it") }
